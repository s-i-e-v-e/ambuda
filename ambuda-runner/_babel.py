...
def init(locale):
    unstd.os.run(["pybabel", "init", "-i", "messages.pot", "-d", "/app/ambuda/translations", f"--locale {locale}"])


# Extract all translatable text from the application and save it in `messages.pot`.
def extract():
    unstd.os.run(["pybabel", "extract", "--mapping", "babel.cfg", "--keywords", "_l", "--output-file", "messages.pot", "."])


# Update all translation files with new text from `messages.pot`
def update():
    unstd.os.run(["pybabel", "update", "-i", "messages.pot", "-d", "/app/ambuda/translations"])
