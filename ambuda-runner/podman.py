...
def build(args: List[str]):
    ...
    unstd.os.run(['podman', 'build', '--tag', cfg.AMBUDA_IMAGE, '--tag', cfg.AMBUDA_IMAGE_LATEST, '-f', 'deploy/Containerfile', '.'])
...
def inspect(args: List[str]):
    unstd.os.run(['podman', 'inspect', cfg.AMBUDA_CONTAINER_NAME, '-f', '{{ .NetworkSettings.IPAddress }} {{ .NetworkSettings.Ports }}'])
