#
# RUNS ON CONTAINER WHEN DEPLOYMENT IS TRIGGERED
#

import sys
sys.path.extend(['./', './ambuda', './unstd'])

from typing import List

import unstd.os
import unstd.config
import _yrun
import db_initialize
import vidyut_initialize
import verify
import code

cfg = unstd.config.read_container_config()


def __start():
    unstd.os.run(['redis-server', '--port', cfg.REDIS_PORT, '--daemonize', 'yes'])
    unstd.os.run(['celery', '-A', 'ambuda.tasks', 'worker', '--detach', '--loglevel=INFO'])

    print('podman: FLASK START')
    print(f"{cfg.FLASK_ENV} Flask start from /venv/bin/flask with {cfg.AMBUDA_CONTAINER_IP} on port 5000")

    if cfg.FLASK_ENV == "development":
        # Start flask server in development mode
        # Dynamically load css and js changes. Docker compose attaches to the ambuda/static directory on localhost.
        unstd.os.run(['./node_modules/.bin/concurrently',
                  f"/venv/bin/flask run -h {cfg.AMBUDA_CONTAINER_IP} -p 5000",
                  "npx tailwindcss -i /app/ambuda/static/css/style.css -o /app/ambuda/static/gen/style.css --watch",
                  "npx esbuild /app/ambuda/static/js/main.js --outfile=/app/ambuda/static/gen/main.js --bundle --watch"
                  ])
    else:
        # Build, Staging, and Production modes take this route. Load site static files that are within the container.
        unstd.os.run(['./node_modules/.bin/concurrently',
                  f"/venv/bin/flask run -h {cfg.AMBUDA_CONTAINER_IP} -p 5000"
                  ])


def __init(args: List[str]):
    print('podman: INIT')
    seed_type = unstd.os.next_arg_pair(args)

    # Initialize SQLite database
    db_initialize.run(cfg, seed_type[1] if seed_type[0] == '--seed' else None)

    # Initialize Vidyut data
    vidyut_initialize.run(cfg)

    __start()


def __main():
    del sys.argv[0]

    cmd = unstd.os.xs_next(sys.argv, 'help')
    switch = {
        'init': __init,
        'check': code.check,
        'test': code.test,
        'lint': code.lint,
        'verify': verify.verify,

        'help': _yrun.help
    }
    f = switch.get(cmd, _yrun.none)
    f(sys.argv)


__main()
